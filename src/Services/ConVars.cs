...
public static class ConVars
{
    ...
    public static readonly IConVar<string> ApiKey = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_apikey",
        "API key for the Inventory Simulator service.",
        ""
    );

    public static readonly IConVar<string> File = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_file",
        "Inventory data file to load when the plugin starts.",
        "inventories.json"
    );

    public static readonly IConVar<bool> IsWsEnabled = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_ws_enabled",
        "Allow players to refresh their inventory using the !ws command.",
        false
    );

    public static readonly IConVar<bool> IsWsImmediately = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_ws_immediately",
        "Apply skin changes immediately without requiring a respawn.",
        false
    );

    public static readonly IConVar<int> WsCooldown = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_ws_cooldown",
    ...
    public static readonly IConVar<bool> IsWsLogin = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_wslogin",
    ...
    public static readonly IConVar<bool> IsSprayEnabled = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_spray_enabled",
        "Enable spraying via the !spray command and/or use key.",
        true
    );

    public static readonly IConVar<bool> IsSprayOnUse = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_spray_on_use",
        "Apply spray when the player presses the use key.",
        false
    );

    public static readonly IConVar<int> SprayCooldown = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_spray_cooldown",
        "Cooldown duration in seconds between sprays per player.",
        30
    );

    public static readonly IConVar<bool> IsSprayChangerEnabled = Swiftly.Core.ConVar.CreateOrFind(
    ...
    public static readonly IConVar<bool> IsFallbackTeam = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_fallback_team",
        "Allow using skins from any team (prioritizes current team first).",
        false
    );

    public static readonly IConVar<int> MinModels = Swiftly.Core.ConVar.CreateOrFind(
        "invsim_minmodels",
